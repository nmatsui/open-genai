...
> **変更履歴:** 初期ローカル化（`fc57e53`）以降の大きな設計変更は [CHANGELOG.md](CHANGELOG.md) を参照してください。
...
- http://localhost:8333/ をブラウザで開くと XML の `AccessDenied` が表示されること（SeaweedFS S3 API の正常応答。管理画面ではなく、利用者は AI アプリ成果物の署名付き URL 経由でファイル取得）
...
へ再アップロードし、**署名付き URL**（既定 24 時間）を outputs に注入します。
`.env` の `S3_*` で設定。未設定時は再ホストをスキップします。

- 画像は従来どおりインライン表示。ドキュメント類はダウンロードリンクとして提示
