...
import { FileLimit, UploadedFileType } from 'genai-web';
...
const useFilesState = create<{
  ...
  getFileDownloadSignedUrl: (s3Url: string, cacheBase64?: boolean) => Promise<string>;
}>((set, get) => {
  ...
  const validateUploadedFiles = async (
    ...
    const isMimeSpoofedResults = await Promise.all(
      uploadedFiles.map(async (uploadedFile) => {
        ...
        // Fix Safari issue: https://github.com/aws-samples/generative-ai-use-cases/pull/1001
        let realMimeType: string | undefined;
        try {
          realMimeType = (await fileTypeFromStream(uploadedFile.file.stream()))?.mime;
        } catch {
          realMimeType = (await fileTypeFromBuffer(await uploadedFile.file.arrayBuffer()))?.mime;
        }
        if (!realMimeType) {
    ...
    const updatedFiles: UploadedFileType[] = uploadedFiles.map((uploadedFile, idx) => {
      ...
      if (uploadedFile.file.type.includes('image')) {
        imageFileCount += 1;
        isFileNumberAllowed = imageFileCount <= (fileLimit.maxImageFileCount || 0);
        if (!isFileNumberAllowed) {
          errorMessages.push(`画像ファイルは ${fileLimit.maxImageFileCount} 個以下にしてください`);
        }
      } else if (uploadedFile.file.type.includes('video')) {
        videoFileCount += 1;
        isFileNumberAllowed = videoFileCount <= (fileLimit.maxVideoFileCount || 0);
        if (!isFileNumberAllowed) {
          errorMessages.push(`動画ファイルは ${fileLimit.maxVideoFileCount} 個以下にしてください`);
        }
      } else {
        fileCount += 1;
        isFileNumberAllowed = fileCount <= (fileLimit.maxFileCount || 0);
        if (!isFileNumberAllowed) {
          errorMessages.push(`ファイルは ${fileLimit.maxFileCount} 個以下にしてください`);
        }
      }
    ...
    return {
      ...
      errorMessages: [
        ...new Set(updatedFiles.flatMap((uploadedFile) => uploadedFile.errorMessages)),
      ],
    };
  };
  ...
  const checkFiles = async (id: string, fileLimit: FileLimit, accept: string[]) => {
    ...
    const { uploadedFiles: newUploadedFiles, errorMessages } = await validateUploadedFiles(
  ...
  // Handle File Uploads
  const uploadFiles = async (id: string, files: File[], fileLimit: FileLimit, accept: string[]) => {
    // Get File
    const currentUploadedFiles = get().uploadedFilesDict[id] ?? [];
    ...
    // Validate File
    const { uploadedFiles: validatedFiles, errorMessages } = await validateUploadedFiles(
      newUploadedFiles,
      fileLimit,
      accept,
    );
  ...
  // getFileDownloadSignedUrl に Base64 キャッシュ機能を追加
  const getFileDownloadSignedUrl = async (s3Url: string, cacheBase64?: boolean) => {
    const url = await fileApi.getFileDownloadSignedUrl(s3Url);
...
export const useFiles = (id: string) => {
  ...
  return {
    ...
    checkFiles: useCallback(
      (fileLimit: FileLimit, accept: string[]) => checkFiles(id, fileLimit, accept),
      [checkFiles, id],
    ),
    errorMessages: errorMessagesDict[id] ?? [],
    clear: () => clear(id),
    uploadedFiles: uploadedFilesDict[id] ?? [],
    deleteUploadedFile: (fileUrl: string, fileLimit: FileLimit, accept: string[]) =>
